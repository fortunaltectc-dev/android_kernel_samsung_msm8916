...
static CLASS_ATTR(tk_enabled, S_IRUGO | S_IWUSR, touch_disabler_get_tk_enabled, touch_disabler_set_tk_enabled);
...
static CLASS_ATTR(ts_enabled, S_IRUGO | S_IWUSR, touch_disabler_get_ts_enabled, touch_disabler_set_ts_enabled);
...
static CLASS_ATTR(tk_control, S_IRUGO | S_IWUSR, touch_disabler_get_tk_control, touch_disabler_set_tk_control);
...
static CLASS_ATTR(ts_control, S_IRUGO | S_IWUSR, touch_disabler_get_ts_control, touch_disabler_set_ts_control);
...
static int touch_disabler_init_sysfs(void)
{
	...
	ret = class_create_file(data->disabler_class, &class_attr_tk_enabled);
	if (ret) {
		pr_err("%s: Failed to create tk_enabled\n", __func__);
		goto err_create_tk_enabled;
	}

	ret = class_create_file(data->disabler_class, &class_attr_ts_enabled);
	if (ret) {
		pr_err("%s: Failed to create ts_enabled\n", __func__);
		goto err_create_ts_enabled;
	}

	ret = class_create_file(data->disabler_class, &class_attr_tk_control);
	if (ret) {
		pr_err("%s: Failed to create tk_control\n", __func__);
		goto err_create_tk_control;
	}

	ret = class_create_file(data->disabler_class, &class_attr_ts_control);
	if (ret) {
...
err_create_ts_control:
	class_remove_file(data->disabler_class, &class_attr_tk_control);
err_create_tk_control:
	class_remove_file(data->disabler_class, &class_attr_ts_enabled);
err_create_ts_enabled:
	class_remove_file(data->disabler_class, &class_attr_tk_enabled);
...
static void touch_disabler_free_sysfs(void)
{
	...
	class_remove_file(data->disabler_class, &class_attr_ts_enabled);
	class_remove_file(data->disabler_class, &class_attr_ts_control);
